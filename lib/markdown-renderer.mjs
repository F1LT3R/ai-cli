...
// chalk (used by cli-highlight) detects color support at import time.
// Force it on unconditionally — the renderer gates styling via its own tty/noColor flags.
...
const supportsOSC8 = () => {
	const tp = String(process.env.TERM_PROGRAM || '')
	return tp !== 'Apple_Terminal' && tp.toLowerCase() !== 'vscode'
}

const renderLink = ({ url, visible }) => {
	const blue = fg256(33)
	const colored = `${SGR.underline}${blue}${visible}${SGR.reset}`
	if (supportsOSC8()) {
		return osc8Link({ url, visible: colored })
	}
	return colored
}
...
const styleInline = ({ text, tty, noColor }) => {
	...
	out = out.replace(/\[([^\]]+)\]\(([^)]+)\)/gu, (_m, label, url) => {
		const blue = fg256(33)
		const rendered = renderLink({ url, visible: url })
...
const styleLineNormal = ({ line, tty, noColor, width }) => {
	...
	out = out.replace(/^(#{1,6})\s+(.*)$/u, (_m, hashes, text) => {
		...
		const deco = level === 1 ? SGR.bold + SGR.underline : SGR.bold
	...
	#processLine = (line) => {
		...
		if (this.mode === 'normal') {
			...
			if (fenceOpen) {
				...
				// Don't enter fence mode for markdown — render content as normal
				// so inner code fences (e.g. ```python) are handled properly
