...
import { parseOptions } from '../lib/options-i1.mjs'
import { shapeRequestBody } from '../lib/response-shape-i1.mjs'
// CLI options normalization
const __argv = process.argv.slice(2)
let __formatValue = null
for (let i = 0; i < __argv.length; i++) {
	if (__argv[i] === '--format' && i + 1 < __argv.length) {
		__formatValue = __argv[i + 1]
		i++
		continue
	}
}
const __normalized = parseOptions({
	argv: __argv.map(a => (__formatValue && a === '--format' ? `--format=${__formatValue}` : a)),
...
const openaiFetch = async ({ apiKey, body, baseUrl }) => {
	...
	const res = await fetch(url, {
		...
		body: JSON.stringify(shapeRequestBody({ format: __normalized.format, body })),
	})
	if (!res.ok || !res.body) {
